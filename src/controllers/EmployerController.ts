...
import { Employer, CreateEmployerBody, Locals, UpdateEmployerBody } from '../types'
...
export class EmployerController {
    ...
    public async getByID(id: number): Promise<Employer> {
        await this.hasAccess(id)

        return this.repository.findOne({ where: { id }, relations: { user: true } })
    }

    public async create(body: CreateEmployerBody): Promise<Employer> {
    ...
    public async update(id: number, body: UpdateEmployerBody): Promise<Employer> {
        await this.hasAccess(id)
